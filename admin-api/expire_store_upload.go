...
func (s *ApiService) ExpireStoreUpload(ctx context.Context, uploadId string, storeId string) (openapi.ImplResponse, error) {
	...
	).UpdateAll(ctx, tx, models.M{models.StoreUploadColumns.Status: models.StoreUploadStatusExpired})
	...
	// Mark upload-files in upload as expired, and clear their file IDs & hash IDs
	_, err = models.StoreUploadFiles(
		qm.Where(models.StoreUploadFileColumns.UploadID+" = ?", upload.UploadID),
	).UpdateAll(ctx, tx, models.M{
		models.StoreUploadFileColumns.Status: models.StoreUploadFileStatusExpired,
