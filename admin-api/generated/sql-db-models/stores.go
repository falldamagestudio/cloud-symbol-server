...
func Stores(mods ...qm.QueryMod) storeQuery {
	mods = append(mods, qm.From("\"cloud_symbol_server\".\"stores\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"cloud_symbol_server\".\"stores\".*"})
	}
...
func FindStore(ctx context.Context, exec boil.ContextExecutor, storeID int, selectCols ...string) (*Store, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"cloud_symbol_server\".\"stores\" where \"store_id\"=$1", sel,
	)
...
func (o *Store) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		wl = strmangle.SetComplement(wl, storeGeneratedColumns)
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"cloud_symbol_server\".\"stores\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"cloud_symbol_server\".\"stores\" %sDEFAULT VALUES%s"
		}
...
func (o *Store) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"cloud_symbol_server\".\"stores\" SET %s WHERE %s",
...
func (o StoreSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"cloud_symbol_server\".\"stores\" SET %s WHERE %s",
...
func (o *Store) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, storeGeneratedColumns)
		update = strmangle.SetComplement(update, storeGeneratedColumns)
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"cloud_symbol_server\".\"stores\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Store) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"cloud_symbol_server\".\"stores\" WHERE \"store_id\"=$1"
...
func (o StoreSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"cloud_symbol_server\".\"stores\" WHERE " +
...
func (o *StoreSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"cloud_symbol_server\".\"stores\".* FROM \"cloud_symbol_server\".\"stores\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, storePrimaryKeyColumns, len(*o))
...
func StoreExists(ctx context.Context, exec boil.ContextExecutor, storeID int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"cloud_symbol_server\".\"stores\" where \"store_id\"=$1 limit 1)"
